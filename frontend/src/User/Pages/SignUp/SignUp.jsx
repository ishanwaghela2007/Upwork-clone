...
const SignUp = () => {
  ...
  const { user, setUser } = useContext(UserContext); // Assuming setUser is a function to set user data in context
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASEURL_URL}/api/auth/register`,
      formData
      );
      if (response.status === 201) {
        ...
        setErrors({ server: "Registration failed. Please check your input." });
      }
    } catch (error) {
      if (error.response?.data?.message) {
        setErrors({ server: error.response.data.message });
      } else if (error.response?.data?.status === "fail" && error.response?.data?.message === "Validation failed") {
        setErrors({ server: "Validation failed" });
      } else {
